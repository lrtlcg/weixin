 ...
 */
function init_tab(tabsId,menu_iter,menu_name) {
  // 1) 初始化tabs
  ...
  $('.'+menu_iter).click(function(){
	  var menuUrl=$("#"+this.id).children('.menu_name').attr('data-id');
	//  alert(menuUrl);
    ...
    // index没有对应的tab  则新增
    if (index==-2){
      addTab(tabs,$(this).find('.'+menu_name).text(),this.id,tabTemplate,menuUrl);
    }
    // 4)初始化关闭
 ...
 */
function addTab(tabs,tabTitle,id,tabTemplate,menuUrl) {
 ...
 // tabContentHtml可以不用在页面写什么内容了,页面内容单独一个url.然后将url通过菜单传入,这里通过嵌入iframe显示url内容
    tabs.append( "<div id='" + id + "' style='height:100%;weight:100%;'> <iframe src='" +menuUrl + "'width='100%' height='100%' frameborder='0' scrolling='no'></iframe></div>" );
...
function inputFormatter(value, row, index){
  ...
  //操作栏的格式化
    ...
    //勾选设置
    function gouFormatter(value,row,index) {
    	if(row.checks!='0'){
    		return{
    			//disabled:true,
    			checked:true
